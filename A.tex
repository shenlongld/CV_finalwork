...
本项目抬头率部分没有重新训练 YOLO 模型，而是直接使用源码中的 yolo-face 和 yolo-head。yolo-face 对应 \texttt{yolov10n-face.pt}，用于检测可见人脸。yolo-head 对应 \texttt{yolov10n-head.pt}，用于检测学生头部。系统先分别得到人脸框和头部框，再判断人脸中心是否落在头部框内。匹配成功的头部记为抬头，未匹配到人脸的头部记为低头。

源码中两个模型的默认路径和下载地址如下，未涉及本项目重新训练：

\begin{lstlisting}[language=Python, caption=YOLO 模型配置源码]
...
\subsection{抬头率计算流程}
抬头率计算流程针对前置摄像机视频。系统按固定间隔抽帧，将同一帧图像分别送入 yolo-face 和 yolo-head。yolo-face 输出人脸框，yolo-head 输出头部框。随后系统对两组框进行匹配，若人脸中心落在某个头部框内，则该头部判为抬头；否则判为低头。该方法没有单独训练抬头/低头分类模型，而是利用人脸可见性和头部位置关系完成状态判断。
...
窗口平滑不会改变检测类别，只减少短时抖动。最终输出包括逐帧统计表、叠加检测框的示例图和整堂课抬头率曲线。核心匹配源码如下：

\begin{lstlisting}[language=Python, caption=人脸与头部匹配源码]
...
检测统计由同一帧的两个模型输出共同生成。源码先运行 yolo-face 和 yolo-head，再调用匹配函数得到抬头标记，最后计算抬头率：

\begin{lstlisting}[language=Python, caption=抬头率统计源码]
...
检测叠加图中，绿色框表示匹配到人脸的头部，橙色框表示未匹配到人脸的头部。头部框来自 yolo-head，人脸框来自 yolo-face。左上角同步显示时间、抬头人数、学生总数、人脸数量和抬头率。检测框和统计值保持对应，便于检查单帧结果。

状态判定以人脸框和头部框的匹配关系为依据。人脸中心落入头部框时记为抬头，没有匹配到人脸的头部记为低头。人脸数量用于观察正脸可见程度，最终抬头率按匹配后的头部状态计算。
