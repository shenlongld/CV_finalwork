...
\subsection{抬头率计算流程}
抬头率计算流程针对前置摄像机视频。系统按固定间隔抽帧，将帧图像送入 YOLO 检测模型。模型输出头部框、类别和置信度，类别包括抬头和低头。后处理阶段先按置信度筛选检测框，再用非极大值抑制去掉重复框。保留下来的框即为当前帧参与统计的学生头部目标。
...
while ok:
    if i % gap == 0:
        res = model(img, conf = 0.35, iou = 0.5, verbose = False)[0]
        up = 0
        down = 0
...
检测叠加图中，绿色框表示抬头，橙色框表示低头。左上角同步显示时间、抬头人数、学生总数、人脸数量和抬头率。检测框和统计值保持对应，便于检查单帧结果。

状态判定以头部区域外观为依据。正向头部通常判为抬头，俯向桌面的头部判为低头。人脸数量只作为辅助记录，最终抬头率按头部状态计算。
